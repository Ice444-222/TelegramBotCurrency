...
async def handle_start(message):
    ...
    await message.answer(text="Также вы можете сделать запрос по конкретной валюте, посмотреть курс на конкретную дату и также увидеть курс Московской Биржы если данная валюта там торгуется. 🤑", reply_markup=get_on_start_kb())
...
async def handle_curs_all(message):
    await message.answer(text=dp.get_curs_all(), reply_markup=base_keyboard.get_on_start_kb())
